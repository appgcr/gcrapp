/**
 * CANONICAL PROPERTY METADATA SCHEMA & NORMALIZATION ENGINE
 * 
 * Defines the canonical metadata format shared by:
 * - 100% Free Local Document AI & Clarity Engine (OpenCV + EasyOCR + pypdfium2)
 * - Born-digital PDF stream extractors
 * - Cross-document reconciliation engine
 * - React Property Details form state
 */

// Field provenance builder
function createField(value = null, sourceDocument = null, sourcePage = null, confidence = 0, evidence = null, status = 'EMPTY', extra = {}) {
  return {
...
// Generates a blank canonical property metadata object
function createEmptyCanonicalMetadata() {
  return {
    ...
    // Advanced Legal OCR Fields
    vendors: createField([]),
    aadharNumbers: createField([]),
    stampPapers: createField([]),
    witnesses: createField([]),
    documentPreparedBy: createField(null),
    correctionDetails: createField(null),
...
function normalizeSurveyNo(val) {
  if (isStrictlyEmpty(val)) return null;
...
function normalizePlotNo(val) {
  if (isStrictlyEmpty(val)) return null;
  const str = String(val).trim();
  // Filter out door numbers mistakenly captured as plot numbers (e.g. 58/384-2-1-2 or 3/838-6-1-2)
...
function normalizeRoadWidth(val) {
  if (isStrictlyEmpty(val)) return null;
  const str = String(val).trim();
  const numMatch = str.match(/([0-9]+(?:\.[0-9]+)?)/);
  if (!numMatch) return null;
...
function normalizePropertyType(val) {
  if (isStrictlyEmpty(val)) return null;
...
function normalizeStructureType(val) {
  if (isStrictlyEmpty(val)) return null;
  const str = String(val).trim().toLowerCase();
...
function cleanBoundaryText(val) {
  ...
  // Check for known OCR artifact patterns like "| joint south: | sont |" or "sont"
  if (/sont/i.test(s) || /joint\s*south/i.test(s)) {
    return null;
  }
  // Strip OCR pipe garbage and leading/trailing punctuation
  s = s.replace(/^[\|\s\:\-]+/, '').replace(/[\|\s\:\-]+$/, '').trim();
  s = s.replace(/^(?:north|south|east|west|ఉత్తరం|దక్షిణం|తూర్పు|పడమర)\s*[:\-\.]\s*/i, '');
  s = s.replace(/[\|\:]+/g, ' ').replace(/\s+/g, ' ').trim();
  // Discard pure OCR noise
